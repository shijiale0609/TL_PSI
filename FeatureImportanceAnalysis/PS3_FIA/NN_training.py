...
print ("y shape before preprocess", y.shape)

#X = preprocessing.StandardScaler().fit_transform(X)
y = preprocessing.StandardScaler().fit_transform(y.reshape(-1, 1)).reshape(len(X),)

print ("X shape after preprocess", X.shape)
print ("y shape after preprocess", y.shape)
...
#checkpoint = torch.load("checkpoint_PS1.pt")
...
plt.figure(figsize=(6,6))
plt.plot(losses_epochs_train, c='blue', label='Train')
plt.plot(losses_epochs_val, c='red',label='Valid')
plt.legend()
plt.savefig("Loss.png")


plt.figure(figsize=(6,6))
plt.plot(scores_epochs_train, c='blue', label = "Train")
plt.plot(scores_epochs_val, c='red', label = "Valid")
plt.ylim(0,1)
plt.legend()
...
np.save("score_train.npy", np.array(scores_epochs_train))
np.save("score_valid.npy", np.array(scores_epochs_val))

np.save("loss_train.npy", np.array(losses_epochs_train))
np.save("loss_valid.npy", np.array(losses_epochs_val))
...
def score(X,y):
	...
	outputs = model_best(inputs.float()).view(-1,)
	score = r2_score(labels.data.numpy(), outputs.data.numpy())
	return score

base_score, score_decreases = get_score_importances(score, X_test,y_test)
feature_importances = np.mean(score_decreases, axis=0)
np.save("base_score.npy", base_score )
np.save("score_decreases.npy", score_decreases)
